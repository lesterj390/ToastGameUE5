...
ATP_ThirdPersonCharacter::ATP_ThirdPersonCharacter()
{
	...
	// Finding HideWidget
	static ConstructorHelpers::FClassFinder<UUserWidget> HideWidgetFinder(TEXT("/Game/_Main/UI/InteractionWidget"));
	if (!ensure (HideWidgetFinder.Class != nullptr)) return;

	HideWidgetSubclass = HideWidgetFinder.Class;
	// Creating HideWidget
	HideWidget = CreateWidget<UUserWidget>(GetWorld(), HideWidgetSubclass);
...
void ATP_ThirdPersonCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Interact", IE_Pressed, this, &ATP_ThirdPersonCharacter::OnAction);
...
void ATP_ThirdPersonCharacter::BeginPlay()
{
	...
	//UE_LOG(LogTemp, Warning, TEXT("Found widget : %s"), *HideWidgetSubclass->GetName());


	if (HideWidget)
	{
		HideWidget->AddToViewport();
	}
...
void ATP_ThirdPersonCharacter::Tick(float DeltaTime) 
{
	...
	if (GetWorld()->LineTraceSingleByChannel(Hit, Start, End, ECC_Visibility, CollisionParams)) {
		if (Hit.bBlockingHit) {
			if (Hit.GetActor()->GetClass()->IsChildOf(ALocker::StaticClass())) {
				HideWidget->GetWidgetFromName("doorprompt")->SetVisibility(ESlateVisibility::Visible);
				CurrentDoor = Cast<ALocker>(Hit.GetActor());
			}
		}
	}
	else {
		HideWidget->GetWidgetFromName("doorprompt")->SetVisibility(ESlateVisibility::Hidden);
		CurrentDoor = NULL;
	}
}
